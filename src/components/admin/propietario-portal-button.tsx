...
export function PropietarioPortalButton({ propietarioId, propietarioNombre }: Props) {
  ...
  const load = async () => {
    setLoading(true);
    const res = await fetch(`/api/propietarios/${propietarioId}/generar-acceso`);
  ...
  return (
    <>
      <button
        onClick={handleOpen}
        className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 hover:bg-blue-100 text-xs font-semibold transition-colors cursor-pointer"
      >
        ...
        >
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md border border-slate-200">
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-slate-900">Portal del Propietario</p>
                <p className="text-xs text-slate-500 mt-0.5">{propietarioNombre}</p>
              </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <button
                onClick={generate}
                disabled={generating}
                className="w-full flex items-center justify-center gap-2 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white rounded-xl text-sm font-semibold transition-colors cursor-pointer"
              >
                {generating ? (
              ...
              {loading ? (
                ...
                <div className="space-y-2">
                  ...
                  {accesos.map((a) => {
                    const url = `${baseUrl}/propietario/${a.token}`;
